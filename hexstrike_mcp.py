...
def setup_mcp_server(hexstrike_client: HexStrikeClient) -> FastMCP:
    ...
    # ============================================================================
    # NEW WEB SECURITY & VULNERABILITY SCANNING TOOLS (v6.0.2)
    # ============================================================================

    @mcp.tool()
    def bandit_scan(target_file: str = "", target_dir: str = "", recursive: bool = True, output_format: str = "json", severity: str = "low", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Bandit for Python code security scanning.

        Args:
            target_file: Single Python file to scan
        ...
        Returns:
            Python code security scan results
        """
        cmd = "bandit"
        if target_file:
            cmd += f" {target_file}"
        elif target_dir:
            cmd += f" -r {target_dir}" if recursive else f" {target_dir}"
        cmd += f" -f {output_format} -ll" if severity == "low" else f" -f {output_format}"
        if additional_args:
            cmd += f" {additional_args}"

        logger.info(f"🔒 Starting Bandit Python security scan")
        result = hexstrike_client.execute_command(cmd)
        if result.get("success"):
            logger.info(f"✅ Bandit scan completed")
        else:
            logger.error(f"❌ Bandit scan failed")
        return result

    @mcp.tool()
    def semgrep_scan(target_path: str = ".", config: str = "p/owasp-top-ten", output_format: str = "json", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Semgrep for code pattern security scanning (SAST).

        Args:
            target_path: Path to scan
            config: Config to use (p/owasp-top-ten, p/security-audit, etc.)
            output_format: Output format (json, sarif, text)
            additional_args: Additional Semgrep arguments

        Returns:
            Code pattern security scan results
        """
        cmd = f"semgrep --config={config} {target_path} --json"
        if additional_args:
            cmd += f" {additional_args}"

        logger.info(f"🔍 Starting Semgrep code pattern scan")
        result = hexstrike_client.execute_command(cmd)
        if result.get("success"):
            logger.info(f"✅ Semgrep scan completed")
        else:
            logger.error(f"❌ Semgrep scan failed")
        return result

    @mcp.tool()
    def safety_check(requirements_file: str = "", json_output: bool = True, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Safety to check Python dependencies for known vulnerabilities.

        Args:
            requirements_file: requirements.txt file path
        ...
        if requirements_file:
            cmd += f" --file {requirements_file}"
        if json_output:
            cmd += " --json"
        if additional_args:
            cmd += f" {additional_args}"

        logger.info(f"🛡️ Starting Safety dependency vulnerability check")
        result = hexstrike_client.execute_command(cmd)
        if result.get("success"):
    ...
    @mcp.tool()
    def truffleHog_scan(target: str, scan_type: str = "filesystem", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute TruffleHog for secret scanning in repositories and files.

        Args:
            target: Target URL (git repo) or filesystem path
            scan_type: Type of scan (filesystem, github, gitlab)
            additional_args: Additional TruffleHog arguments

        Returns:
            Secret discovery results
        """
        if scan_type == "filesystem":
            cmd = f"trufflehog filesystem {target} --json"
        else:
            cmd = f"trufflehog {scan_type} --repo-path {target} --json"
        ...
        result = hexstrike_client.execute_command(cmd)
        if result.get("success"):
            logger.info(f"✅ TruffleHog scan completed")
        else:
            logger.error(f"❌ TruffleHog scan failed")
        return result

    @mcp.tool()
    def snyk_test(target_path: str = ".", severity: str = "low", format_type: str = "json", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Snyk for dependency and vulnerability testing.

        Args:
            target_path: Path to test
            severity: Minimum severity (low, medium, high, critical)
            format_type: Output format (json, sarif, table)
            additional_args: Additional Snyk arguments

        Returns:
            Dependency vulnerability test results
        """
        cmd = f"snyk test {target_path} --{format_type}"
        if severity:
            cmd += f" --severity-threshold={severity}"
        if additional_args:
            cmd += f" {additional_args}"

        logger.info(f"📦 Starting Snyk dependency vulnerability test")
        result = hexstrike_client.execute_command(cmd)
        if result.get("success"):
            logger.info(f"✅ Snyk test completed")
        else:
            logger.error(f"❌ Snyk test failed")
        return result

    @mcp.tool()
    def pip_audit_check(additional_args: str = "") -> Dict[str, Any]:
        """
        Execute pip-audit to audit Python packages for known vulnerabilities.

        Args:
            additional_args: Additional pip-audit arguments

        Returns:
            Python package vulnerability audit results
        """
        cmd = "pip-audit --format json"
        if additional_args:
            cmd += f" {additional_args}"

        logger.info(f"🐍 Starting pip-audit vulnerability check")
        result = hexstrike_client.execute_command(cmd)
        if result.get("success"):
            logger.info(f"✅ pip-audit check completed")
        else:
            logger.error(f"❌ pip-audit check failed")
        return result
